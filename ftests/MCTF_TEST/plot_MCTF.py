...
def skim_gofs(file_names):
    """
    Extract 'limit' values from ROOT files and store them in a dictionary.
    
    Parameters:
    file_names (list): List of ROOT file names to skim.
    
    Returns:
    dict: A dictionary with keys as modified filenames and entry indices, and values as 'limit' values.
    """
    out = {}
...
def collate_gofs(base_dict, alt_dict):
    """
    Collate 'limit' values from two dictionaries into two arrays.
    
    Parameters:
    base_dict (dict): Dictionary of 'limit' values for base toys.
    alt_dict (dict): Dictionary of 'limit' values for alternative toys.
    
    Returns:
    tuple: Two numpy arrays containing the 'limit' values.
    """
    bar, altr = [], []
...
def fval(lambda1, lambda2, p1, p2, nbins):
    """
    Calculate the F-value given two sets of lambda values and parameters.
    
    Parameters:
    lambda1 (float): Lambda value for base.
    lambda2 (float): Lambda value for alternative.
    p1 (int): Number of parameters for base.
    p2 (int): Number of parameters for alternative.
    nbins (int): Number of bins.
    
    Returns:
    float: Calculated F-value.
    """
