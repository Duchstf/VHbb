...
def table_1():
    ...
    mu_VH_up   = [2.52, 2.20, 1.26, 1.22, 0.75]
    mu_VH_down = [2.23, 1.87, 1.18, 1.10, 0.71]

    mu_VH_low  = [m - d for m, d in zip(mu_VH.values, mu_VH_down)]
    mu_VH_high = [m + u for m, u in zip(mu_VH.values, mu_VH_up)]

    mu_VH_unc = Uncertainty("mu_VH uncertainty", is_symmetric=False)
    mu_VH_unc.set_values_from_intervals(zip(mu_VH_low, mu_VH_high), nominal=mu_VH.values)
    ...
    mu_VZ_err = [1.77, 1.53, 1.28, 0.92, 0.63]  # symmetric

    mu_VZ_low  = [m - e for m, e in zip(mu_VZ.values, mu_VZ_err)]
    mu_VZ_high = [m + e for m, e in zip(mu_VZ.values, mu_VZ_err)]

    mu_VZ_unc = Uncertainty("mu_VZ uncertainty", is_symmetric=False)
    mu_VZ_unc.set_values_from_intervals(zip(mu_VZ_low, mu_VZ_high), nominal=mu_VZ.values)
