...
def plot_syst_allbkg(h, g, syst, xtitle, title, name):
    ...
    charm_hists = []
    charm_hists += [h.integrate('process','Wjets').integrate('genflavor',int_range=slice(1,3))]
    charm_hists += [h.integrate('process','Zjets').integrate('genflavor',int_range=slice(1,3))]
    charm_hists += [h.integrate('process','Zjets').integrate('genflavor',int_range=slice(3,4))]
    charm_hists += [h.sum('genflavor').integrate('process',p) for p in mc]

    light_hists = []
    light_hists += [g.integrate('process','Wjets').integrate('genflavor',int_range=slice(1,3))]
    light_hists += [g.integrate('process','Zjets').integrate('genflavor',int_range=slice(1,3))]
    light_hists += [g.integrate('process','Zjets').integrate('genflavor',int_range=slice(3,4))]
    light_hists += [g.sum('genflavor').integrate('process',p) for p in mc]
