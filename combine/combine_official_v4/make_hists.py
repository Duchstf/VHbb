...
def main():
    ...
    for region in regions:
        ...
        sig_0 =  pickle.load(open(pickle_path,'rb')).integrate('region','signal').integrate('systematic', 'nominal').sum('pt1', overflow='all').sum('genflavor1', overflow='all')
        sig = pickle.load(open(pickle_path,'rb')).integrate('region','signal').integrate('systematic', 'nominal').integrate().integrate('cc2',int_range=c_int_range)
