...
if [ "$1" == "unblind" ]; then
    conda run -n plot --no-capture-output combine_postfits -i fitDiagnosticsTest.root $add_all -o plots/test_plot --data --style ../files/style_D.yml --onto qcd --sigs VH --bkgs QCD,qcd,top,WjetsQQ,Zjets,Zjetsbb,VqqVqq,H,WLNu,VbbVqq --xlabel 'Jet 1 $m_{SD}$ [GeV]'  --rmap 'VH:rVH' -p --format both --cmslabel '' --lumi '138'
    conda run -n plot --no-capture-output combine_postfits -i fitDiagnosticsTest.root -o plots/test_plot --data --style ../files/style_D.yml --onto qcd --sigs VH --bkgs QCD,qcd,top,WjetsQQ,Zjets,Zjetsbb,VqqVqq,VbbVqq,H,WLNu  --xlabel 'Jet 1 $m_{SD}$ [GeV]' -p  --rmap 'VH:rVH'
