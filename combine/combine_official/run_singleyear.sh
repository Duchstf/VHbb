...
if [[ "$2" == "unblind" || "$2" == "unblind_sideband" ]]; then
    conda run -n plot --no-capture-output combine_postfits -i fitDiagnosticsTest.root -o plots/test_plot --data --style ../files/style_D.yml --onto qcd --bkgs QCD,qcd,ttbar,singlet,WjetsQQ,Zjets,Zjetsbb,VVNLO,H,WLNu  --xlabel 'Jet 1 $m_{SD}$ [GeV]' -p 
else
