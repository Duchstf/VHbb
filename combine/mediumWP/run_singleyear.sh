...
region_file="/uscms_data/d3/dhoang/VH_analysis/CMSSW_10_2_13/src/VHbb/output/mediumWP/$1/regions.root"
...
if [[ "$2" == "unblind" || "$2" == "unblind_sideband" ]]; then
    conda run -n plot --no-capture-output combine_postfits -i fitDiagnosticsTest.root -o plots/test_plot --data --style ../files/style_D.yml --onto qcd --bkgs QCD,qcd,top,WjetsQQ,Zjets,Zjetsbb,H,WLNu  --xlabel 'Jet 1 $m_{SD}$ [GeV]' -p 
else
