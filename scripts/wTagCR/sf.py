...
def test_sfmodel(fittype='single', scale=1, smear=0.1, template_dir={}, fit_dir=None):
    ...
    for template, region in regions:
        ...
        wqq_templ = get_templ(template_dir[template], 'matched_{}'.format(region), 'nominal')
        wqq_sample = rl.TemplateSample("{}_wqq".format(ch.name), rl.Sample.SIGNAL, wqq_templ)
        wqq_sample.setParamEffect(sys_scale, 
                                  get_templ(template_dir[template], 'matched_{}'.format(region), 'scaleUp', False),
                                  get_templ(template_dir[template], 'matched_{}'.format(region), 'scaleDown', False),
                                  scale=scale,
                                  )
        wqq_sample.setParamEffect(sys_smear, 
                                  get_templ(template_dir[template], 'matched_{}'.format(region), 'smearUp', False),
                                  get_templ(template_dir[template], 'matched_{}'.format(region), 'smearDown', False),
                                  scale=smear,
                                  )
        ...
        qcd_templ = get_templ(template_dir[template], 'unmatched_{}'.format(region), 'nominal')
