...
def plot_flavor_stack(h, process='QCD', stack_axis='genflavor1', bb_pass =True):
    ...
    include_label = 'qcdpass' if bb_pass else 'all'
    include_title = f'{p_name} QCD Pass' if bb_pass else p_name
    ...
    plt.savefig(f'plotsQCDCut/{process}_{msd_label}_{include_label}.pdf', bbox_inches='tight')
...
def plot_flavor(h, p):
    ...
    h_plot_1_all = h.sum('qcd2','msd2', 'genflavor2', overflow='all').integrate('process', p)
    h_plot_1_pass = h.sum('msd2', 'genflavor2', overflow='all').integrate('process', p).integrate('qcd2', slice(0.0, 0.0741))

    h_plot_2_all = h.sum('qcd2', 'msd1', 'genflavor1', overflow='all').integrate('process', p)
    h_plot_2_pass = h.sum('msd1', 'genflavor1', overflow='all').integrate('process', p).integrate('qcd2', slice(0.0, 0.0741))
...
def main():
    ...
    h = pickle.load(open(pickle_path,'rb')).integrate('region','signal').integrate('systematic', 'nominal').sum('bb1')
