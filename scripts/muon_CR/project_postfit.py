...
def plot_h(h, labels, name, year):
    ...
    #Get the ratio
    if name == 'bb_pass':
        ratio = post_fit_ratio(year, region="pass")
    else:
        ratio = post_fit_ratio(year, region="fail")
    print(f"ratio ({name}): ", ratio)
    ...
    for hist_data in mc_hists:
        hist_data.view().value *= ratio
        hist_data.view().variance *= ratio ** 2
    ...
    plt.savefig(f'plots/{year}_muCR_{name}_postfit.pdf', bbox_inches='tight')
    # plt.savefig(f'plots/{year}_muCR_{name}_postfit.png', bbox_inches='tight')    

def post_fit_ratio(year, region="pass"):

    fit_data = uproot.open(f"fit/{year}/fitDiagnosticsTest.root")

    prefit = fit_data[f'shapes_prefit/muonCR{region}{year}/total'].values()[0]
    postfit = fit_data[f'shapes_fit_s/muonCR{region}{year}/total'].values()[0]
